...
$.fn.wu = function (options, files) {
    ...
    var addFile = function (file, isUrl) {
        ...
        if (!isPDF) {
            ...
            if (isUrl) {
                ...
                $imgWrap.html('<a href="'+file.url+'" class="file-path" target="_blank"><img src="' + file.url + '"></a>')
            }
        }
    ...
    uploader.on( 'uploadProgress', function( file, percentage ) {
        ...
        // 避免重复创建
        if ( !$percent.length ) {
            $percent = $('<div class="progress ">' +'<div style="width: 0%" class="progress-bar">' +'</div>' +'</div>').appendTo( $li ).find('.progress-bar');
        }
    ...
    uploader.on('uploadSuccess', function (file, res) {
        ...
        if ( (res.status &&res.status != 0) || (res.code && res.code != 0)) {
            ...
            !isPreview && $imgWrap.html('<a href="javascript:;" class="file-path" target="_blank"><img src="' + res.path + '"></a>')
