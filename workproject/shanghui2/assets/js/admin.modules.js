...
app.run(["$rootScope","$state","resource" ,"$templateCache","CompanyScaleList","INDUSTRY",function(root,state,resource,$templateCache,CompanyScaleList,INDUSTRY) {
  ...
  root.ReadAuthList = [{id:1,text:'同乡'},{id:2,text:'会员'},{id:3,text:'服务组'},{id:4,text:'到期会员'}];
  root.all_config = {
  ...
  // 获取城市
  root.cityData = {};
  root.cityDataFn = function(code){
      var ret = [];
      var _code = code?code:86;
      var districts = ChineseDistricts[_code];
  ...
  root.cityDataInit = function(){
    var ret = [];
    var districts = ChineseDistricts[86];
    var getDistrict = function(code){
        var districts = ChineseDistricts[code] || {};
        var data = [];
        $.each(districts, function (code, address) {
    ...
    $.each(districts, function (code, address) {
        ...
        for (var i = 0; i < address.length; i++) {
              ...
              root.cityData[prov.text] = getCity(prov.code);
              provs.push(prov);
        }
        
            
        ret = ret.concat(provs);    
    });
    root.provinceList = ret;
    return ret;
  };
  root.cityDataInit();
