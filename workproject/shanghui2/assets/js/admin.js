 ...
 * By Daryl Rowland
 * https://github.com/majinhui04/angucomplete
 */
...
angular.module('angucomplete', [] )
    ...
    return {
        ...
        link: function($scope, elem, attrs) {
            //console.warn('angucomplete',elem)
            ...
            $scope.pause = 500;
            $scope.minLength = 3;
            $scope.searchStr = $scope.selectedObject? $scope.selectedObject.title:null;
            console.log('selectedObject',$scope.selectedObject,121,$scope.$root.$$phase);
                    ...
                    if ($scope.localData) {
                        ...
                        $http.get($scope.url + str, {}).
                            success(function(responseData, status, headers, config) {
                                ...
                                $scope.searching = false;
                                list = (($scope.dataField) ? responseData[$scope.dataField] : responseData );
                                //console.log(222,list,responseData.data.list,str)
                                list = list&&list.length?list:responseData.data.list;
                                $scope.processResults(list, str);
                            }).
                            error(function(data, status, headers, config) {
                                console.log("error");
                            });
