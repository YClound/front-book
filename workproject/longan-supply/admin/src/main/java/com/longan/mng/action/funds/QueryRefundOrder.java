...
public class QueryRefundOrder extends BaseController {
	...
	public void index(@ModelAttribute("refundOrderQuery") RefundOrderQuery refundOrderQuery,
	...
	public void doQuery(@ModelAttribute("refundOrderQuery") RefundOrderQuery refundOrderQuery,
			Model model, HttpSession session) {
		UserInfo userInfo = super.getUserInfo(session);
		setSelectValue(userInfo, model, refundOrderQuery);
		...
		if (refundOrderQuery.getPayOrderId() != null || refundOrderQuery.getId() != null
			...
			if (!check2Date(refundOrderQuery.getStartGmtCreate(),
					refundOrderQuery.getEndGmtCreate())) {
	...
	public Map<String, String> statusList() {
		...
		map.put(Constants.RefundOrder.STATUS_SUCCESS + "",
				Constants.RefundOrder.STATUS_SUCCESS_DESC);
		map.put(Constants.RefundOrder.STATUS_UNCONFIRMED + "",
				Constants.RefundOrder.STATUS_UNCONFIRMED_DESC);
		return map;
	}
	...
	public Map<String, String> payTypeList() {
		Map<String, String> map = new HashMap<String, String>();
		map.put(Constants.RefundOrder.PAY_TYPE_OPERATOR + "",
				Constants.RefundOrder.PAY_TYPE_OPERATOR_DESC);
		map.put(Constants.RefundOrder.PAY_TYPE_SYSTEM + "",
				Constants.RefundOrder.PAY_TYPE_SYSTEM_DESC);
		return map;
	}
