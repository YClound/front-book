...
public class BizOrderManDeal extends BaseController {
	...
	public String lockRequest(@RequestParam("bizOrderId") Long bizOrderId,
			@RequestParam("posId") String posId,
			@ModelAttribute("bizOrderQuery") BizOrderQuery bizOrderQuery, HttpSession session,
			Model model) {
	...
	public String unlockRequest(@RequestParam("bizOrderId") Long bizOrderId,
		...
		if (updateResult.isSuccess() && updateResult.getModule()) {
			// 解锁就减1
			decrCountInMemcached(bizOrder.getItemExt1());
	...
	public String cancelRequest(@RequestParam("bizOrderId") Long bizOrderId,
		...
		if (bizOrder.getStatus() != Constants.BizOrder.STATUS_CHARGING
				&& bizOrder.getStatus() != Constants.BizOrder.STATUS_LOCK
		...
		if (updateResult.isSuccess() && updateResult.getModule()) {
			...
			try {
				...
				if (!callBackResult.isSuccess() || !callBackResult.getModule()) {
					logger.error("cancelBizOrder callback failed bizOrderId : " + bizOrderId,
	...
	public String confirmRequest(@RequestParam("bizOrderId") Long bizOrderId,
		...
		if (updateResult.isSuccess() && updateResult.getModule()) {
			...
			try {
				Result<Boolean> callBackResult = callBackService.callBackAsync(bizOrder);
				if (!callBackResult.isSuccess() || !callBackResult.getModule()) {
					logger.error("confirmBizOrder callback failed bizOrderId : " + bizOrderId,
							callBackResult.getResultMsg());
				}
			} catch (Exception e) {
	...
	private String formToParam(BizOrderQuery bizOrderQuery) {
		...
		for (int i = 0; i < field.length; i++) {
			...
			try {
				if (field[i].get(bizOrderQuery) != null
						&& !field[i].getName().equals("serialVersionUID")) {
					if (field[i].get(bizOrderQuery) instanceof Date) {
						sb.append(field[i].getName()).append("=")
								.append(DateTool.parseDate((Date) field[i].get(bizOrderQuery)))
								.append("&");
					} else {
						sb.append(field[i].getName()).append("=")
								.append(field[i].get(bizOrderQuery)).append("&");
					}
				}
