...
public class QueryOperationInfo extends BaseController {
	...
	public void index(@ModelAttribute("operationInfoQuery") OperationInfoQuery operationInfoQuery,
			Model model) {
	}

	@RequestMapping(method = RequestMethod.POST)
	public void doQuery(
			@ModelAttribute("operationInfoQuery") OperationInfoQuery operationInfoQuery, Model model) {
		Map<Integer, String> firstMenuMap = firstMenuList();
		Result<List<OperationInfo>> result = operationInfoService
				.queryOperationInfoList(operationInfoQuery);
		if (result.isSuccess() && result.getModule() != null) {
			...
			for (OperationInfo operationInfo : list) {
				if (StringUtils.isNotEmpty(operationInfo.getBelongMenu() + "")) {
					operationInfo
							.setBelongMenuDesc(firstMenuMap.get(operationInfo.getBelongMenu()));
				}
			}
