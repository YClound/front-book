...
public class ItemSupplyDeal extends BaseController {
	...
	public String onRequestUp(@RequestParam("id") Long id, @RequestParam("bizId") Integer bizId,
		...
		if (result.isSuccess()) {
			...
			OperationLog operationLog = OperLogUtils.operationLogDeal(older, itemService
					.getItemSupply(id).getModule(), userInfo, map.get("moduleName") + "(上架)", older
					.getBizId(), map.get("loginIp"));
			operationLogService.createOperationLog(operationLog);
			alertSuccess(model, refererUrl.replace("biz/", "") + ".do?bizId=" + bizId + "&id=" + id);
		} else {
	...
	public String onRequestDown(@RequestParam("id") Long id, @RequestParam("bizId") Integer bizId,
			HttpSession session, Model model, HttpServletRequest request) {
		String refererUrl = getRefererUrlUnPath(request);
		UserInfo userInfo = super.getUserInfo(session);
		...
		if (result.isSuccess()) {
			...
			Map<String, String> map = (HashMap<String, String>) session
					.getAttribute("requestInfoMap");
			OperationLog operationLog = OperLogUtils.operationLogDeal(older, itemService
					.getItemSupply(id).getModule(), userInfo, map.get("moduleName") + "(下架)", older
					.getBizId(), map.get("loginIp"));
			operationLogService.createOperationLog(operationLog);
			alertSuccess(model, refererUrl.replace("biz/", "") + ".do?bizId=" + bizId + "&id=" + id);
		} else {
	...
	public String onPostAdd(@ModelAttribute("itemSupplyForm") ItemSupplyForm itemSupplyForm,
			BindingResult bindingResult, HttpSession session, Model model) {
		UserInfo userInfo = super.getUserInfo(session);
		model.addAttribute("bizName",
				Constants.BIZ_MAP.get(Integer.parseInt(itemSupplyForm.getBizId())));
		...
		if (result.isSuccess()) {
			...
			operationLogService.createOperationLog(operationLog);
			if (itemSupply.getBizId() == 500) {
				super.alertSuccess(model,
						"queryGameItemSupply.do?bizId=" + itemSupplyForm.getBizId() + "&id="
								+ itemSupply.getId());

			} else {
				super.alertSuccess(model, "queryItemSupply.do?bizId=" + itemSupplyForm.getBizId()
						+ "&id=" + itemSupply.getId());
			}
		} else {
			super.alertError(model, result.getResultMsg());
	...
	public String onListAddIndex(@RequestParam("bizId") Integer bizId,
			@RequestParam("ids") String ids, HttpSession session, Model model,
	...
	public String onListAdd(
			@ModelAttribute("itemSupplyListForm") ItemSupplyListForm itemSupplyListForm,
		...
		if (i > 0) {
			...
			OperationLog operationLog = OperLogUtils.operationLogDeal(description, userInfo,
					...
					Constants.OperationLog.TYPE_ADD);
			operationLogService.createOperationLog(operationLog);
			super.alertSuccess(model, refererUrl.replace("biz/", "") + ".do?bizId="
					+ itemSupplyListForm.getBizId());
		} else {
	...
	public String onListEditIndex(@RequestParam("bizId") Integer bizId,
			@RequestParam("ids") String ids, HttpSession session, Model model,
			HttpServletRequest request) {
		String refererUrl = getRefererUrlUnPath(request);
		model.addAttribute("refererUrl", refererUrl);
	...
	public String onRequestConditionEditIndex(
			@ModelAttribute("itemSupplyQuery") ItemSupplyQuery itemSupplyQuery,
			@RequestParam("bizId") Integer bizId, @RequestParam("requestType") String requestType,
			HttpSession session, Model model, HttpServletRequest request) {
		String refererUrl = getRefererUrlUnPath(request);
		model.addAttribute("refererUrl", refererUrl);
	...
	public String onListEdit(
			@ModelAttribute("itemSupplyListEditForm") ItemSupplyListEditForm itemSupplyListEditForm,
			@RequestParam("refererUrl") String refererUrl, BindingResult bindingResult,
			HttpSession session, Model model) {
		UserInfo userInfo = super.getUserInfo(session);
		...
		logger.warn(userInfo.getUserName() + "执行批量修改供货，  供货商品 ids : "
				+ itemSupplyListEditForm.getIds());
		...
		if (i > 0) {
			...
			operationLogService.createOperationLog(operationLog);
			super.alertSuccess(model, refererUrl.replace("biz/", "") + ".do?bizId="
					+ itemSupplyListEditForm.getBizId());
		} else {
	...
	public String onPostEdit(@ModelAttribute("itemSupplyForm") ItemSupplyForm itemSupplyForm,
			BindingResult bindingResult, HttpSession session, Model model) {
		UserInfo userInfo = super.getUserInfo(session);
		model.addAttribute("bizName",
				Constants.BIZ_MAP.get(Integer.parseInt(itemSupplyForm.getBizId())));
		...
		Result<ItemSupply> itemSupplyResult = itemService.getItemSupply(Long
				.parseLong(itemSupplyForm.getId()));
		if (!itemSupplyResult.isSuccess()) {
			super.alertError(model, itemSupplyResult.getResultMsg());
			return "biz/itemSupplyEdit";
		}
		...
		OperationLog operationLog = OperLogUtils.operationLogDeal(older, itemSupply, userInfo,
				map.get("moduleName"), itemSupply.getBizId(), map.get("loginIp"));
		operationLogService.createOperationLog(operationLog);
		super.alertSuccess(model, "queryItemSupply.do?bizId=" + itemSupplyForm.getBizId() + "&id="
				+ itemSupply.getId());
		return "biz/itemSupplyEdit";
	}
	...
	public String onRequestConditionDownIndex(
			@ModelAttribute("itemSupplyQuery") ItemSupplyQuery itemSupplyQuery,
			@RequestParam("bizId") Integer bizId, @RequestParam("requestType") String requestType,
			HttpSession session, Model model, HttpServletRequest request) {
		String refererUrl = getRefererUrlUnPath(request);
	...
	public String onRequestBatchDown(@ModelAttribute("upDownForm") UpDownForm upDownForm,
		...
		Result<Long> result = super.submitTaskForm(upDownForm, "itemService", "downItemSupply",
	...
	public String onRequestConditionUpIndex(
			@ModelAttribute("itemSupplyQuery") ItemSupplyQuery itemSupplyQuery,
			@RequestParam("bizId") Integer bizId, @RequestParam("requestType") String requestType,
			HttpSession session, Model model, HttpServletRequest request) {
		String refererUrl = getRefererUrlUnPath(request);
	...
	public String onRequestBatchUp(@ModelAttribute("upDownForm") UpDownForm upDownForm,
		...
		Result<Long> result = super.submitTaskForm(upDownForm, "itemService", "upItemSupply",
				(Serializable) list, "java.util.List", operationLog, userInfo,
				Integer.parseInt(upDownForm.getBizId()));
	...
	private ItemSupply formToItemSupply(ItemSupplyForm itemSupplyForm) {
		...
		if (StringUtils.isNotEmpty(itemSupplyForm.getSupplyProductCode())) {
	...
	private ItemSupply formToItemSupplyEdit(ItemSupplyListEditForm itemSupplyListEditForm,
			Long itemSupplyId) {
		ItemSupply result = new ItemSupply();
	...
	public Map<String, String> supplyTypeList() {
		...
		mapType.put(Constants.ItemSupply.TYPE_CARD_FORWARD_CHARGE + "",
				Constants.ItemSupply.TYPE_CARD_FORWARD_CHARGE_DESC);
		mapType.put(Constants.ItemSupply.TYPE_DIRECT_CHARGE + "",
				Constants.ItemSupply.TYPE_DIRECT_CHARGE_DESC);
		...
		mapType.put(Constants.ItemSupply.TYPE_CARD_CHARGE + "",
				Constants.ItemSupply.TYPE_CARD_CHARGE_DESC);
		return mapType;
	}
	...
	public Map<String, String> asyncSupplyList() {
		Map<String, String> mapType = new HashMap<String, String>();
		mapType.put(Constants.TraderInfo.SUPPLY_WAY_SYNC + "",
				Constants.TraderInfo.SUPPLY_WAY_SYNC_DESC);
		mapType.put(Constants.TraderInfo.SUPPLY_WAY_ASYNC + "",
				Constants.TraderInfo.SUPPLY_WAY_ASYNC_DESC);
		return mapType;
	}
