...
public class CashOrderDeal extends BaseController {
	...
	public String onRequestPass(@RequestParam("cashOrderId") Long cashOrderId, HttpSession session,
			Model model) {
		...
		if (result.isSuccess() && result.getModule()) {
			...
			OperationLog operationLog = OperLogUtils.operationLogDeal(older, cashOrderService
					.getCashOrder(cashOrderId).getModule(), userInfo, map.get("moduleName")
	...
	public String onRequestCancel(@RequestParam("cashOrderId") Long cashOrderId,
