...
public class QueryPayOrder extends BaseController {
	...
	public void index(@ModelAttribute("payOrderQuery") PayOrderQuery payOrderQuery, Model model,
			HttpSession session) {
	...
	public void doQuery(@ModelAttribute("payOrderQuery") PayOrderQuery payOrderQuery, Model model,
			HttpSession session) {
		UserInfo userInfo = super.getUserInfo(session);
		setSelectValue(model, userInfo, payOrderQuery);
	...
	public Map<String, String> statusList() {
		...
		map.put(Constants.PayOrder.STATUS_UNCONFIRMED + "",
				Constants.PayOrder.STATUS_UNCONFIRMED_DESC);
