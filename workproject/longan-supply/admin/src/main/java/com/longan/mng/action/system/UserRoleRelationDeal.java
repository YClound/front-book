...
public class UserRoleRelationDeal extends BaseController {
	...
	public String onPostAdd(@RequestParam("requestParam") String requestParam,
			@RequestParam("userId") Long userId, HttpSession session, Model model) {
		...
		try {
			Result<Boolean> result = commitUserRoleRelationDao.EditUserRoleRelation(
					userRoleRelationList, userId);
			if (result.isSuccess()) {
