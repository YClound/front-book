...
public class ItemPriceExcelView extends AbstractExcelView {
	...
	void buildSheet(Map<String, Object> model, HSSFWorkbook workbook, List<ItemPrice> result,
			int sheetIndex) {
		...
		header.createCell(9).setCellValue("供货商 ");
		header.createCell(10).setCellValue("状态 ");

		int size = 0;
		if (result.size() > MAX_ROW) {
			size = (result.size() - MAX_ROW * (sheetIndex - 1)) > MAX_ROW ? MAX_ROW : (result
					.size() - MAX_ROW * (sheetIndex - 1));
		} else {
			size = result.size();
		}
		...
		for (int i = 0; i < size; i++) {
			...
			if (itemPrice.getPriceDesc(itemPrice.getItemFacePrice()) != null) {
				row.createCell(3).setCellValue(
						Double.parseDouble(itemPrice.getPriceDesc(itemPrice.getItemFacePrice())));// 转为数值型，方便导出后进行计算
			}
			if (itemPrice.getItemCostPriceDiscount() != null) {
				row.createCell(4).setCellValue(
						Double.parseDouble(itemPrice.getItemCostPriceDiscount()));
			}
			if (itemPrice.getItemSalesPriceDiscount() != null) {
				row.createCell(5).setCellValue(
						Double.parseDouble(itemPrice.getItemSalesPriceDiscount()));
			}
			if (itemPrice.getItemSalesPrice2Discount() != null) {
				row.createCell(6).setCellValue(
						Double.parseDouble(itemPrice.getItemSalesPrice2Discount()));
			}
			if (itemPrice.getItemSalesPrice3Discount() != null) {
				row.createCell(7).setCellValue(
						Double.parseDouble(itemPrice.getItemSalesPrice3Discount()));
			}
			if (itemPrice.getItemSalesPrice4Discount() != null) {
				row.createCell(8).setCellValue(
						Double.parseDouble(itemPrice.getItemSalesPrice4Discount()));
			}
			row.createCell(9).setCellValue(itemPrice.getSupplyTraderName());
			row.createCell(10).setCellValue(itemPrice.getStatusDesc());
		}
	}
}
