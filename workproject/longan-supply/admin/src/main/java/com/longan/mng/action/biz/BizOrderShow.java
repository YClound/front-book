...
public class BizOrderShow extends BaseController {
	...
	public String orderShow(@RequestParam("bizOrderId") Long bizOrderId,
			@RequestParam("bizId") int bizId, Model model) {
		...
		if (StringUtils.isNumeric(bizOrder.getUpstreamId())) {
			UserInfo traderUserInfo = localCachedService.getUserInfo(Long.parseLong(bizOrder
					.getUpstreamId()));
			TraderInfo traderInfo = localCachedService.getTraderInfo(Long.parseLong(bizOrder
					.getUpstreamId()));
