...
public class QueryAllBizOrder extends BaseController {
	...
	public void doQuery(@ModelAttribute("bizOrderQuery") BizOrderQuery bizOrderQuery, Model model,
			HttpSession session) {
...
//			}
		...
		if (result.isSuccess()) {
			...
			for (BizOrder bizOrder : list) {
				...
				if (StringUtils.isNotBlank(bizOrder.getProvinceCode())) {
					AreaInfo areaInfo = localCachedService.getProvinceByCode(bizOrder
							.getProvinceCode());
					if (areaInfo != null) {
				...
				if (bizOrder.getSupplyType() == null && bizOrder.getItemSupplyId() != null) {
					ItemSupply itemSupply = localCachedService.getItemSupply(bizOrder
							.getItemSupplyId());
					if (itemSupply != null) {
						// 之后改成itemSupplyType
				...
				if (StringUtils.isNumeric(bizOrder.getUpstreamId())) {
					UserInfo traderUserInfo = localCachedService.getUserInfo(Long
							.parseLong(bizOrder.getUpstreamId()));
		...
		// 订单统计。
		Result<BizOrderStatisic> countBizOrderResult;
		if (userInfo.isDownStreamUser()) {
			countBizOrderResult = queryService.countDownStreamBizOrder(bizOrderQuery);
		} else {
			countBizOrderResult = queryService.countBizOrder(bizOrderQuery);
		}
	...
	public Map<String, String> statusList() {
		...
		map.put(Constants.BizOrder.STATUS_LOCK + "", Constants.BizOrder.STATUS_LOCK_DESC);
		map.put(Constants.BizOrder.STATUS_UNCONFIRMED + "",
				Constants.BizOrder.STATUS_UNCONFIRMED_DESC);
	...
	public Map<String, String> carrierTypeList() {
		...
		mapType.put(Constants.Item.CARRIER_TYPE_MOBILE + "",
				Constants.Item.CARRIER_TYPE_MOBILE_DESC);
		mapType.put(Constants.Item.CARRIER_TYPE_TELECOM + "",
				Constants.Item.CARRIER_TYPE_TELECOM_DESC);
		mapType.put(Constants.Item.CARRIER_TYPE_UNICOM + "",
				Constants.Item.CARRIER_TYPE_UNICOM_DESC);
	...
	public Map<String, String> channelList() {
		Map<String, String> map = new HashMap<String, String>();
		map.put(Constants.BizOrder.CHANNEL_SUPPLY + "", Constants.BizOrder.CHANNEL_SUPPLY_DESC);
		map.put(Constants.BizOrder.CHANNEL_WEB + "", Constants.BizOrder.CHANNEL_WEB_DESC);
		return map;
	}
}
