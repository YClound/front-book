...
import com.longan.mng.api.response.ManualBizOrderResponse;

@Controller
public class ManualBizOrder extends BaseController {
	@Resource
	...
	private static final long ROBOT_ID = Long.parseLong(Utils.getProperty("robotId"));

	@RequestMapping(value = "api/lockBizOrder")
	public @ResponseBody
	ManualBizOrderResponse onRequestLock(
			@ModelAttribute("lockBizOrderRequest") LockBizOrderRequest lockBizOrderRequest,
			BindingResult bindingResult, HttpSession session, Model model,
			HttpServletRequest request) {
		LockBizOrderResponse lockBizOrderResponse = new LockBizOrderResponse();
		logger.warn("接受: ip : " + super.getRemoteIp(request) + " robot锁定订单请求 : "
				+ lockBizOrderRequest);
		...
		Result<BizOrder> result = bizOrderService.robotLockBizOrder(
				Long.parseLong(lockBizOrderRequest.getSupplyTraderId()),
	...
	public @ResponseBody
	ManualBizOrderResponse onRequestCallBack(
		...
		logger.warn("接受: ip : " + super.getRemoteIp(request) + " robot结果返回请求 : "
				+ callbackBizOrderRequest);
		...
		if (STATUS_SUCCESS.equals(callbackBizOrderRequest.getStatus())) {
			...
			mimiMonitorService.countByPosId(bizOrder.getItemExt1(), bizOrder.getAmount());
			...
			bizOrder.setUpstreamSerialno(callbackBizOrderRequest.getNo());
			bizOrder.setItemExt3(callbackBizOrderRequest.getPwd());
	...
	private ManualBizOrderResponse getErrorResponse(BindingResult bindingResult) {
		ManualBizOrderResponse result = new ManualBizOrderResponse();
	...
	private void callback(BizOrder bizOrder) {
		...
		try {
			...
			if (!callBackResult.isSuccess() || !callBackResult.getModule()) {
				logger.error("callback failed bizOrderId : " + bizOrder.getId(),
