...
public class BizOrderExcel2007View extends AbstractExcel2007View {
	...
	void buildSheet(Map<String, Object> model, SXSSFWorkbook workbook, List<BizOrder> result, int sheetIndex, boolean isDownStream) {
		Sheet sheet = workbook.createSheet(sheetIndex + ""); // 创建表格
		Row header = sheet.createRow(0);// 定义表格的表头
		...
		int size = 0;
		if (result.size() > MAX_ROW) {
			size = (result.size() - MAX_ROW * (sheetIndex - 1)) > MAX_ROW ? MAX_ROW : (result.size() - MAX_ROW * (sheetIndex - 1));
		} else {
			size = result.size();
		}
		...
		for (int i = 0; i < size; i++) {
			...
			if (!isDownStream) {
				...
				row.createCell(12).setCellValue(bizOrder.getItemExt1());
