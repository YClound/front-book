...
public class StockDownload extends BaseController {
	...
	public ModelAndView onRequest(@ModelAttribute("stockLogId") Long stockLogId, HttpSession session) {
		...
		Result<List<Stock>> result = excelExportService.queryStockListByStockLogId(stockLogId,
				userInfo);
		...
		OperationLog operationLog = OperLogUtils.operationLogDeal("stockLogId : " + stockLogId,
				userInfo, map.get("moduleName"), null, map.get("loginIp"),
				Constants.OperationLog.TYPE_OTHER);
