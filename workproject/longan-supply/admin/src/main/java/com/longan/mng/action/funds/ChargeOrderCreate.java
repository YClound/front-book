...
public class ChargeOrderCreate extends BaseController {
	...
	public String onRequestQueryUser(@RequestParam("loginId") String loginId, HttpSession session,
			Model model) {
		...
		if (result.isSuccess() && result.getModule() != null) {
			...
			if (userInfo.getType() != null
					&& userInfo.getType() == Constants.UserInfo.TYPE_DOWNSTREAM) {
	...
	public String onPostCreate(@ModelAttribute("chargeOrderForm") ChargeOrderForm chargeOrderForm,
		...
		if (result.isSuccess()) {
			...
			OperationLog operationLog = OperLogUtils.operationLogDeal(null, chargeOrder, userInfo,
