...
public class QueryStock extends BaseController {
	...
	public String index(@ModelAttribute("stockQuery") StockQuery stockQuery, HttpSession session,
			Model model) {
		// 业务权限判断
	...
	public String doQuery(@ModelAttribute("stockQuery") StockQuery stockQuery, Model model,
			HttpSession session) {
		UserInfo userInfo = super.getUserInfo(session);
		// 业务权限判断
		if (!checkBizAuth(stockQuery.getBizId(), userInfo)) {
		...
		if (stockQuery.getId() != null || stockQuery.getBizOrderId() != null
				|| stockQuery.getInSerialno() != null || stockQuery.getOutSerialno() != null) {
	...
	void setSelectValue(UserInfo userInfo, Model model, StockQuery stockQuery) {
		...
		Result<List<ItemSupply>> itemSupplyResult = itemService.queryItemSupplyByBizId(stockQuery
				.getBizId());
	...
	public Map<String, String> statusList() {
		...
		map.put(Constants.Stock.STATUS_INV_ALLOCATED + "",
				Constants.Stock.STATUS_INV_ALLOCATED_DESC);
