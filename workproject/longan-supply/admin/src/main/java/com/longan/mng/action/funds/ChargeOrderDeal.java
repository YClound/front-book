...
public class ChargeOrderDeal extends BaseController {
	...
	public String onRequestPass(@RequestParam("chargeOrderId") Long chargeOrderId,
		...
		if (result.isSuccess() && result.getModule()) {
			...
			OperationLog operationLog = OperLogUtils.operationLogDeal(older, chargeOrderService
					.getChargeOrder(chargeOrderId).getModule(), userInfo, map.get("moduleName")
					+ "(通过)", null, map.get("loginIp"));
	...
	public String onRequestCancel(@RequestParam("chargeOrderId") Long chargeOrderId,
			HttpSession session, Model model) {
		UserInfo userInfo = super.getUserInfo(session);
	...
	public String onPostCreatePre(@RequestParam("loginId") Long loginId, HttpSession session,
			Model model) {
