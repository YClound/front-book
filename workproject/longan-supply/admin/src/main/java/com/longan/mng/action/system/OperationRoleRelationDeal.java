...
public class OperationRoleRelationDeal extends BaseController {
	...
	public String onPostAdd(@RequestParam("requestParam") String requestParam,
			@RequestParam("roleId") Integer roleId, HttpSession session, Model model) {
		...
		try {
			Result<Boolean> result = commitOperationRoleRelationDao.EditOperationRoleRelation(
					operationRoleRelationList, roleId);
			if (result.isSuccess()) {
				@SuppressWarnings("unchecked")
				Map<String, String> map = (HashMap<String, String>) session
						.getAttribute("requestInfoMap");
				OperationLog operationLog = OperLogUtils.operationLogDeal(null, newer, userInfo,
