...
public class RoleInfoDeal extends BaseController {
	...
	public void onPostEdit(@ModelAttribute("roleInfoForm") RoleInfoForm roleInfoForm,
		...
		Result<RoleInfo> roleInfoResult = roleInfoService.getRoleInfoById(Integer
				.parseInt(roleInfoForm.getId()));
		...
		if (result.isSuccess()) {
			...
			OperationLog operationLog = OperLogUtils.operationLogDeal(older, roleInfo, userInfo,
	...
	public void onPostAdd(@ModelAttribute("roleInfoForm") RoleInfoForm roleInfoForm,
			BindingResult bindingResult, HttpSession session, Model model) {
		UserInfo userInfo = super.getUserInfo(session);
		validator.validate(roleInfoForm, bindingResult);
		if (bindingResult.hasErrors()) {
		...
		if (result != null && result.isSuccess()) {
			...
			Map<String, String> map = (HashMap<String, String>) session
					.getAttribute("requestInfoMap");
			OperationLog operationLog = OperLogUtils.operationLogDeal(null, roleInfo, userInfo,
					map.get("moduleName"), null, map.get("loginIp"));
			operationLogService.createOperationLog(operationLog);
			super.alertSuccess(model, "queryRoleInfo.do");
		} else {
