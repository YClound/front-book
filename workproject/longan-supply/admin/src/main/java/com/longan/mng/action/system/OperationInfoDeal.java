...
public class OperationInfoDeal extends BaseController {
	...
	public void onPostEdit(
			@ModelAttribute("operationInfoForm") OperationInfoForm operationInfoForm,
		...
		Result<OperationInfo> operationInfoResult = operationInfoService
				.getOperationInfoById(Integer.parseInt(operationInfoForm.getId()));
		...
		if (result.isSuccess()) {
			...
			OperationLog operationLog = OperLogUtils.operationLogDeal(older, operationInfo,
					userInfo, map.get("moduleName"), null, map.get("loginIp"));
	...
	public void onPostAdd(@ModelAttribute("operationInfoForm") OperationInfoForm operationInfoForm,
			BindingResult bindingResult, HttpSession session, Model model) {
		UserInfo userInfo = super.getUserInfo(session);
		validator.validate(operationInfoForm, bindingResult);
		if (bindingResult.hasErrors()) {
		...
		if (result != null && result.isSuccess()) {
			...
			Map<String, String> map = (HashMap<String, String>) session
					.getAttribute("requestInfoMap");
			OperationLog operationLog = OperLogUtils.operationLogDeal(null, operationInfo,
					userInfo, map.get("moduleName"), null, map.get("loginIp"));
			operationLogService.createOperationLog(operationLog);
			super.alertSuccess(model, "queryOperationInfo.do");
		} else {
	...
	public Map<Integer, String> firstMenuList() {
		...
		Result<List<OperationInfo>> result = operationInfoService
				.queryOperationInfoByType(Constants.OperationInfo.TYPE_CATALOG);
	...
	public Map<String, String> typeList() {
		...
		mapType.put(Constants.OperationInfo.TYPE_CATALOG + "",
				Constants.OperationInfo.TYPE_CATALOG_DESC);
	...
	public Map<String, String> statusList() {
		...
		map.put(Constants.OperationInfo.STATUS_NORMAL + "",
				Constants.OperationInfo.STATUS_NORMAL_DESC);
		map.put(Constants.OperationInfo.STATUS_CANCEL + "",
				Constants.OperationInfo.STATUS_CANCEL_DESC);
