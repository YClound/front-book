...
public class ItemPriceDeal extends BaseController {
	...
	public String onAddIndex(@RequestParam("itemSupplyId") Long itemSupplyId, HttpSession session,
			Model model) {
	...
	public String onAddEdit(@ModelAttribute("itemPriceForm") ItemPriceForm itemPriceForm,
		...
		logger.warn(userInfo.getUserName() + "执行商品调价操作 供货商品 id : "
				+ itemPriceForm.getItemSupplyId());

		Result<ItemSupply> itemSupplyResult = itemService.getItemSupply(Long
				.parseLong(itemPriceForm.getItemSupplyId()));
		...
		OperationLog operationLog = OperLogUtils.operationLogDeal(oldItemPrice, itemPrice,
				userInfo, getModuleNameFromSession(session), item.getBizId(),
				getIpFromSession(session));

		Result<Long> result = super.submitTaskForm(itemPriceForm, "itemPriceService",
				"adjustPrice", (Serializable) itemPrice, itemPrice.getClass().getName(),
				operationLog, userInfo, item.getBizId());
		if (!result.isSuccess()) {
	...
	public String onListAddIndex(@RequestParam("itemSupplyIdList") String itemSupplyIdList,
		...
		for (Long itemSupplyId : list) {
			...
			Item item = itemResult.getModule();
			itemNameList = itemNameList + item.getItemName() + "("
					+ itemSupply.getSupplyTraderName() + ")" + " 、";
		}
	...
	public String onListAddEdit(
			@ModelAttribute("itemPriceListForm") ItemPriceListForm itemPriceListForm,
			BindingResult bindingResult, HttpSession session, Model model) {
		final String returnUrl = "biz/itemPriceListEdit";
		UserInfo userInfo = super.getUserInfo(session);
		logger.warn(userInfo.getUserName() + "执行商品调价操作 供货商品编号列表 : "
				+ itemPriceListForm.getItemSupplyIdList());
		...
		Result<Long> result = super.submitTaskForm(itemPriceListForm, "itemPriceService",
				"batchAdjustPrice", (Serializable) itemPriceList, "java.util.List", operationLog,
				userInfo, null);
