...
public class OperLogUtils {
	...
	public static OperationLog operationLogDeal(Object older, Object newer, UserInfo userInfo,
			String moduleName, Integer bizId, String loginIp) {// 没有bizId则为null
		try {
	...
	public static OperationLog operationLogDeal(String description, UserInfo userInfo,
			String moduleName, Integer bizId, String loginIp, Integer operType) {// 没有bizId则为null
		try {
			OperationLog operationLog = new OperationLog();
			operationLog.setBizId(bizId);
			operationLog.setDescription(description);
	...
	public static String diff(Object older, Object newer) {
		...
		for (Field field : fields) {
			...
			if ((methodName.length() > 2 // 过滤不显示的字段
					...
					|| (methodName.length() > 3 && "time".equalsIgnoreCase(methodName.substring(
							methodName.length() - 4, methodName.length())))
					|| "pwd".equalsIgnoreCase(methodName)
			...
			try {
				...
				if ((null != property1 && !property1.equals(property2))
						|| (null != property2 && !property2.equals(property1))) {
	...
	public static String getInfo(Object obj) throws Exception {
		...
		for (Field field : fields) {
			methodName = field.getName();
			/*
			 * if (methodName.equalsIgnoreCase("id") || (methodName.length() > 3
			 * && "name".equalsIgnoreCase(methodName.substring(
			 * methodName.length() - 4, methodName.length())))) {
			 */
			if ((methodName.length() > 3 && ("list".equalsIgnoreCase(methodName.substring(
					methodName.length() - 4, methodName.length())) || "type"
					.equalsIgnoreCase(methodName.substring(methodName.length() - 4,
							methodName.length()))))
					|| (methodName.length() > 4 && "bizId".equalsIgnoreCase(methodName.substring(
							methodName.length() - 5, methodName.length())))
					|| "pwd".equalsIgnoreCase(methodName)) {
				continue;
			}
			methodName = "get" + methodName.substring(0, 1).toUpperCase() + methodName.substring(1);
