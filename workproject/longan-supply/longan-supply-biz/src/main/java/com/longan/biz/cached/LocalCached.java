...
public abstract class LocalCached<K, V> implements Cached<K, V> {
	...
	private final static ScheduledExecutorService ses = new ScheduledThreadPoolExecutor(5);
	...
	public void reload() {
		...
		logger.warn(this.getClass().getName() + " reload from db cost : "
		...
		try {
			...
			logger.warn(this.getClass().getName() + " reload cost : "
					+ (System.currentTimeMillis() - timeStart) + "ms");
		} finally {
