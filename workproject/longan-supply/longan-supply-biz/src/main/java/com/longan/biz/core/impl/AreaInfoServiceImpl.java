	...
	public Result<Boolean> importDb(File file) {
		...
		try {
			...
			while ((str = br.readLine()) != null && !("").equals(str)) {
				...
				if (StringUtils.isBlank(mobileBelong.getMobilePart())) {
					logger.warn("this row mobilePart isEmpty");
					continue;
				}
				MobileBelongExample example = new MobileBelongExample();
				example.createCriteria().andMobilePartEqualTo(mobileBelong.getMobilePart());
				int count = mobileBelongDAO.countByExample(example);
				if (count == 0) {
					if (mobileBelong.getMobilePart().length() > 7) {
						logger.warn("too long for insert : " + mobileBelong.getMobilePart() + " " + mobileBelong.getMobileTypeName());
						continue;
					}
