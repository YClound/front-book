...
public class QueryAcctLogDAOImpl implements QueryAcctLogDAO {
	...
	public List<AcctLog> queryByPage(AcctLogQuery acctLogQuery) throws SQLException {
		int count = (Integer) querySqlMapClient.queryForObject("acct_log.queryForPageCount",
				acctLogQuery);
		acctLogQuery.setTotalItem(count);
		return (List<AcctLog>) querySqlMapClient
				.queryForList("acct_log.queryForPage", acctLogQuery);
	}

	@SuppressWarnings("unchecked")
	@Override
	public List<AcctLog> queryByExport(AcctLogQuery acctLogQuery) throws SQLException {
		return (List<AcctLog>) querySqlMapClient.queryForList("acct_log.queryByExport",
				acctLogQuery);
	}
}