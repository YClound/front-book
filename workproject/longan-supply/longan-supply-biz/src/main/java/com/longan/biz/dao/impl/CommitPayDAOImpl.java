...
public class CommitPayDAOImpl implements CommitPayDAO {
	...
	public Result<Boolean> deductMoney(Long payOrderId) throws Exception {
		...
		example.createCriteria().andStatusEqualTo(Constants.PayOrder.STATUS_INIT)
				.andIdEqualTo(payOrder.getId());
	...
	public Result<Boolean> addMoney(ChargeOrder chargeOrder) throws Exception {
		...
		example.createCriteria().andIdEqualTo(chargeOrder.getId())
				.andStatusEqualTo(Constants.ChargeOrder.STATUS_INIT);
	...
	public Result<Boolean> refundMoney(Long refundOrderId) throws Exception {
		...
		example.createCriteria().andIdEqualTo(refundOrder.getId())
				.andStatusEqualTo(Constants.RefundOrder.STATUS_INIT);
		int refundOrderUpdateCount = refundOrderDAO.updateByExampleSelective(refundOrderUpdate,
				example);
		...
		payOrderExample.createCriteria().andIdEqualTo(payOrder.getId())
				.andStatusEqualTo(Constants.PayOrder.STATUS_SUCCESS);
	...
	@Override
	@Transactional(readOnly = false, propagation = Propagation.REQUIRED, rollbackFor = Exception.class)
	public Result<Boolean> adjustPayOrder(Long refundOrderId, OperationVO operationVO)
			throws Exception {
		Result<Boolean> result = new Result<Boolean>();
		result.setModule(false);
		RefundOrder refundOrder = refundOrderDAO.selectByPrimaryKey(refundOrderId);
		...
		Result<List<SupplyOrder>> supplyOrderQuery = supplyOrderService
				.querySupplyOrderByBizOrder(bizOrder.getId());
			...
			for (SupplyOrder supplyOrder : supplyOrderList) {
				if (supplyOrder.canAdjust()) {
					if (supplyOrder.isSupplyFromStock()) {
						Result<Boolean> adjustStockResult = stockService.adjustStorage(supplyOrder
								.getStockId());
						if (!adjustStockResult.isSuccess()) {
							// 不影响主流程
							logger.error("调整库存失败  stockId : " + supplyOrder.getStockId()
									+ " msg : " + adjustStockResult.getResultMsg());
						}
					}
					supplyOrder.setSupplyStatus(Constants.SupplyOrder.STATUS_FAILED);
					Result<Boolean> updateSupplyOrder = supplyOrderService
							.updateSupplyOrder(supplyOrder);
