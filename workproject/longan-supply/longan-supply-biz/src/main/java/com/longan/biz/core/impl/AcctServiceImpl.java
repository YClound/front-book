...
public class AcctServiceImpl extends BaseService implements AcctService {
	...
	public String getVerificationCode(AcctInfo acctInfo) {
		String sourceStr = (acctInfo.getLastTradeBalance() == null ? 0 : acctInfo
				.getLastTradeBalance())
				+ SPLIT
	...
	@Override
	public Result<Boolean> checkBalanceAcct(AcctInfo acctInfo, Item item, int amt) {
		Result<Boolean> result = new Result<Boolean>();
		result.setModule(false);
		Result<Integer> priceResult = itemService.getPriceByAcct(item, acctInfo);
		Integer price = null;
		if (priceResult.isSuccess()) {
			price = priceResult.getModule();
		} else {
			result.setResultMsg(priceResult.getResultMsg());
			return result;
		}
		result.setSuccess(true);
		boolean flag = acctInfo.getBalance() != null && acctInfo.getBalance() > (price * amt);
		result.setModule(flag);
		if (!flag) {
	...
	public Result<Boolean> addMoney(ChargeOrder chargeOrder) {
		...
		try {
			...
			AsyncUpdate asyncUpdate = new AsyncUpdate(Constants.AcctLog.BILL_TYPE_CHARGE_ORDER,
					chargeOrder.getId(), e.getMessage());
	...
	public Result<Boolean> refundMoney(Long refundOrderId) {
		...
		try {
			...
			AsyncUpdate asyncUpdate = new AsyncUpdate(Constants.AcctLog.BILL_TYPE_REFUND_ORDER,
					refundOrderId, e.getMessage());
			es.execute(asyncUpdate);
		}
		return result;
	}
	...
	public Result<Boolean> subMoney(CashOrder cashOrder) {
		...
		try {
			...
			AsyncUpdate asyncUpdate = new AsyncUpdate(Constants.AcctLog.BILL_TYPE_CASH_ORDER,
					cashOrder.getId(), e.getMessage());
			es.execute(asyncUpdate);
		}
		return result;
	}
