...
public class StockServiceImpl extends BaseService implements StockService {
	...
	public Result<Stock> lockStock(SupplyOrder supplyOrder) {
		...
		try {
			...
			logger.error(
					"lockStock error bizOrderId : " + supplyOrder.getBizOrderId()
							+ " supllyOrderId : " + supplyOrder.getId() + " itemSupplyId : "
							+ supplyOrder.getItemSupplyId(), e);
		}
	...
	public Result<Boolean> deliveryFromStorage(Stock stock) {
		...
		try {
			...
			example.createCriteria().andIdEqualTo(stock.getId())
					.andStatusEqualTo(Constants.Stock.STATUS_INV_ALLOCATED);
			...
			logger.error("deliveryFromStorage error stockId : " + stock.getId() + " itemId : "
					+ stock.getItemId(), e);
		}
		return result;
	}
	...
	public Result<Boolean> returnToStorage(Stock stock) {
		...
		if (!stock.canReturnToStorage()) {
			result.setResultMsg("库存返还操作，库存状态必须是出库中或 异常隔离状态的库存");
			return result;
		}
		try {
			...
			logger.error(
					"returnToStorage error stockId : " + stock.getId() + " itemId : "
							+ stock.getItemId(), e);
		}
		return result;
	}
	...
	@Override
	public Result<StockLog> createInStockLog(Long itemSupplyId, Integer itemCostPrice,
			Integer bizId, Long operId) {
	...
	public Result<Boolean> sequestrationStorage(Stock stock) {
		...
		try {
			...
			logger.error("sequestrationStorage error stockId : " + stock.getId() + " itemId : "
					+ stock.getItemId(), e);
		}
		return result;
	}
	...
	public Result<Boolean> adjustStorage(Long stockId) {
		...
		if (stock.getStatus() != Constants.Stock.STATUS_INV_ALLOCATED
				&& stock.getStatus() != Constants.Stock.STATUS_DELIVERY) {
		...
		try {
			...
			logger.error(
					"adjustStorage error stockId : " + stock.getId() + " itemId : "
							+ stock.getItemId(), e);
		}
		return result;
	}
