...
public class LocalBizAuthCached extends LocalCached<Long, Set<Integer>> {
	...
	void reloadFromDb(ConcurrentMap<Long, Set<Integer>> cached) {
		...
		try {
			List<UserInfo> userInfoList = (List<UserInfo>) userInfoDAO
					.selectByExample(userInfoExample);
			if (userInfoList != null) {
				for (UserInfo userInfo : userInfoList) {
					Result<Set<Integer>> result = authService
							.getAuthBizIdByUserId(userInfo.getId());
					if (result.isSuccess()) {
						cached.put(userInfo.getId(), result.getModule());
					} else {
						logger.error("getAuthBizIdByUserId error userId : " + userInfo.getId()
