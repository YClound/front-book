...
public class ExcelExportServiceImpl extends BaseService implements ExcelExportService {
	...
	private ExecutorService downStreamExceutor = new ThreadPoolExecutor(1, 1, 0L,
			TimeUnit.MILLISECONDS, new LinkedBlockingQueue<Runnable>(1));

	// 最多2个线程同时进行导出处理，最多8个队列，超过队列数则报拒绝异常,内部人员要宽松一点
	private ExecutorService exceutor = new ThreadPoolExecutor(1,1, 0L, TimeUnit.MILLISECONDS,
	...
	@Override
	public Result<List<BizOrder>> queryBizOrderExport(final BizOrderQuery bizOrderQuery,
		...
		Callable<Result<List<BizOrder>>> callable = new Callable<Result<List<BizOrder>>>() {
			...
			public Result<List<BizOrder>> call() throws Exception {
				return queryService.queryBizOrderExport(bizOrderQuery);
			}
	...
	@Override
	public Result<List<RefundOrder>> queryRefundOrderExport(
			final RefundOrderQuery refundOrderQuery, Boolean isDownStream) {
	...
	@Override
	public Result<List<AcctLog>> queryAcctLogExport(final AcctLogQuery acctLogQuery,
			Boolean isDownStream) {
		...
		Callable<Result<List<AcctLog>>> callable = new Callable<Result<List<AcctLog>>>() {
			...
			public Result<List<AcctLog>> call() throws Exception {
				return queryService.queryAcctLogExport(acctLogQuery);
			}
	...
	public Result<List<Stock>> queryStockListByStockLogId(Long stockLogId, UserInfo userInfo) {
		...
		Result<List<Stock>> queryStockPageResult = queryService.queryStockPage(stockQuery);
	...
	public Result<List<SupplyOrder>> querySupplyOrderExport(final SupplyOrderQuery supplyOrderQuery) {
		...
		Callable<Result<List<SupplyOrder>>> callable = new Callable<Result<List<SupplyOrder>>>() {
			...
			public Result<List<SupplyOrder>> call() throws Exception {
				return queryService.querySupplyOrderExport(supplyOrderQuery);
			}
