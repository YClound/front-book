...
public class ItemPriceDAOImpl implements ItemPriceDAO {
	...
	public List<ItemPrice> queryByPage(ItemPriceQuery itemPriceQuery) throws SQLException {
		int count = (Integer) sqlMapClient.queryForObject("item_price.queryByPageCount",
				itemPriceQuery);
		itemPriceQuery.setTotalItem(count);
		return (List<ItemPrice>) sqlMapClient
				.queryForList("item_price.queryByPage", itemPriceQuery);
	}
