		...
		try {
			...
			if (list != null) {
				for (UserRoleRelation userRoleRelation : list) {
					List<OperationInfo> operationInfoList = operationInfoDAO
							.selectOperationListByRole(userRoleRelation.getRoleId(),
									Constants.OperationInfo.TYPE_CATALOG);
		...
		try {
			...
			if (list != null) {
				for (UserRoleRelation userRoleRelation : list) {
					List<OperationInfo> operationInfoList = operationInfoDAO
							.selectOperationListByRole(userRoleRelation.getRoleId(),
									Constants.OperationInfo.TYPE_URL);
					for (OperationInfo operationInfo : operationInfoList) {
		...
		try {
			...
			if (list != null) {
				for (UserRoleRelation userRoleRelation : list) {
					List<OperationInfo> operationInfoList = operationInfoDAO
							.selectOperationListByRole(userRoleRelation.getRoleId(),
									Constants.OperationInfo.TYPE_BIZ);
					for (OperationInfo operationInfo : operationInfoList) {
