...
public class TaskServiceImpl extends BaseService implements TaskService {
	...
	private final static ExecutorService taskEs = Executors.newFixedThreadPool(4);
	...
	private Result<Boolean> realTimeSubmitTask(Task task) {
		...
		if (service == null) {
			logger.error("realTimeSubmitTask error service is null, serviceName : "
					+ task.getServiceName() + "  methodName: " + task.getMethodName()
					+ " description: " + task.getInstanceDesc());
		...
		try {
			...
			logger.error(
					"method invoke error serviceName : " + task.getServiceName() + "  methodName: "
							+ task.getMethodName() + " description: " + task.getInstanceDesc(), e);
