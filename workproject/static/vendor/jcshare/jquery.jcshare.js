...
var TPL = '<div class="modal fade" id="qrcodeModal" tabindex="-1" role="dialog" >'+
              '<div class="modal-dialog qrcode" >'+
                '<div class="modal-content">'+
                  '<div class="modal-header">'+
                    '<button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>'+
                    '<h4 class="modal-title">“扫一扫”将网页分享至朋友圈</h4>'+
                  '</div>'+
                  '<div class="modal-body tac">'+
                    '<div id="qrcode" class="dib" style="min-height:200px;"></div>'+
                  '</div>'+
...
$.Share = function(input,options){
    ...
    $.each(options.sharePlace, function(name, tag){
        ...
        input.find(tag).each(function(_,el){
            var _link = $(this);
            if(_link.hasClass('share_wechat')){
                $('body').append(TPL);
                // 二维码
...
$.Share.defaults = {
    ...
    target: "_blank",
    replace : {
        wechat : function(div, title, content, url){
            return "";
        },
