...
var tab = {
    //index.html,topic.html页面tab转换
    tabListFocus : function(){
        if($(".tab-list-focus").length > 0){
        var locationHref = window.location.href;
        var locationValue = locationHref.split("?");
        var value = locationValue[1].split("&");
        console.log(value);

        $(".aw-nav-tabs li a").each(function(){
            var liHref = $(this).attr("href").split("?");
            var liHrefValue = liHref[1].split("&");
            for (var j = liHrefValue.length - 1; j >= 0; j--) {
                if(liHrefValue[j].split("=")[0] == "tab"){
                    for (var i = value.length - 1; i >= 0; i--) {
                        if(value[i].split("=")[0] == "tab"){
                            if(liHrefValue[j].split("=")[1] == value[i].split("=")[1]){
...
$(document).ready(function ()
{
    ...
    {
        console.log(typeof (G_NOTIFICATION_INTERVAL));
