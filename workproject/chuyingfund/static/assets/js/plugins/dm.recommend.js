                                    ...
                                    '<div class="modal-content">'+
                                        '<form action="{action}" method="get" class="ajax-form " >'+
                                            ...
                                            '<div class="modal-body" >'+
                                                ...
                                                '<div class="ctrl-wrap check-triangle dib lh-20 mr-7 mb-7">'+
                                                    '<input type="radio" id="r1" name="proid" value="{proid}" checked="checked">'+
                                                    '<label for="r1">{title}<i class="if icon-check-triangle"></i></label>'+
                                                '</div>'+
                                                '<div class="ctrl-wrap" >'+
        ...
        return this.each(function () {
            ...
            var showModal = function(json){
                ...
                if(json.list.length>1){
                    var $first = html.find('.check-triangle').eq(0);
                    var buffer = ['<div>']
                    for(var i=0;i<json.list.length;i++){
                        var item = json.list[i];
                        var id = 'r' + i;
                        var html = '<div class="ctrl-wrap check-triangle dib lh-20 mr-7 mb-7">'+ '<input type="radio" id="'+id+'" name="proid" value="'+item.id+'" >'+ '<label for="'+id+'">'+item.title+'<i class="if icon-check-triangle"></i></label>'+ '</div>'
                        buffer.push(html);
                    }
                    buffer.push('</div>');
