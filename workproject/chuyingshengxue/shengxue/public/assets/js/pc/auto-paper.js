$(function(){
  ...
  var getsubjectData = function(subjectId){
    // 获取题型
    dm.get(location+'api/questionType/list.json',{typeSubjectId:subjectId}).done(function(res){
      var data = res.data || [];
      var html = "";
    ...
    //获取考点
    dm.get(location+'api/syllabus/listAll.json',{syllabusSubjectId:subjectId}).done(function(res){
  ...
  var getSyllabusId = function(){
    var number = 0;
    var syllabusId = [];
    var syllabusList = $('.syllabus-children').find('.syllabus');
  ...
  var init = function(){ 
    var subjectId = serverData.user.subjectId;
    // 获取学科
    dm.get(location+'api/subject/list.json').done(function(res){
    ...
    // 获取难度
    dm.get(location+'api/questionProperty/difficults.json').done(function(res){
      var data = res.data || [];
      var html = "";
      ...
      if(syllabusId == 0){
        ...
        $.each($('.questionType-item'),function(){
          ...
          if(typeCount > 0){
            ...
            var typeSorce = $(this).find('.single-score').val() || 0;
            typeConfig = {
              typeId : typeId,
              count : typeCount,
              sorce : typeSorce
    ...
    $('.auto-form').on('success',function(){
      document.location.href = "test-paper.html";
    })
  }
  init();
})